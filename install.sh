...
# the bunx round-trip and keeps offline installs working. BASH_SOURCE is unset
...
# Curl-pipe path: delegate to bunx. We do NOT pass `--` here — npm 7+ bunx
# already forwards trailing args to the package, and a literal `--` tripped
# bin/install.js's parseArgs as an unknown flag.
if ! command -v bunx >/dev/null 2>&1; then
  echo "caveman: bunx required (ships with Node ≥18). Reinstall Node.js." >&2
  exit 1
fi
