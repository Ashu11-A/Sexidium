...
/**
 * Marks a chest menu's title as a Sexidium menu so the NeoForge <b>client</b> can recognise it and
 * re-skin the screen, without changing what the server sends to anyone else.
 *
 * <p>The NeoForge adapter always opens a plain vanilla {@code GENERIC_9xN} container — that is the
 ...
 *
 * <p>Everything fails safe: if the marker is stripped, mangled, or never read (vanilla/Geyser client),
 * the screen is simply the plain chest. All methods are pure so they can be unit-tested headless.</p>
 */
